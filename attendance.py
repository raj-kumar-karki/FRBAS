...
class Attendance:
    ...
    def __init__(self,root):
        ...
        #title section
        title_lb1 = Label(bg_img,text="Welcome To Attendance Panel",font=("verdana",20,"bold"),bg="white",fg="navyblue")
        ...
        # Creating Frame 
        main_frame = Frame(bg_img,bd=2,bg="#4682b4") #bd mean border 
        main_frame.place(x=5,y=90,width=1355,height=510)

        # Left Label Frame 
        left_frame = LabelFrame(main_frame,bd=2,bg="#4682b4",relief=RIDGE,text="Student Details",font=("verdana",12,"bold"),fg="navyblue")
        ...
        #Student id
        studentId_label = Label(left_frame,text="Std-ID:",font=("verdana",12,"bold"),fg="navyblue",bg="#4682b4")
        ...
        #Student Roll
        student_roll_label = Label(left_frame,text="Roll.No:",font=("verdana",12,"bold"),fg="navyblue",bg="#4682b4")
        ...
        #Studnet Name
        student_name_label = Label(left_frame,text="Std-Name:",font=("verdana",12,"bold"),fg="navyblue",bg="#4682b4")
        ...
        #time
        time_label = Label(left_frame,text="Time:",font=("verdana",12,"bold"),fg="navyblue",bg="#4682b4")
        ...
        #Date 
        date_label = Label(left_frame,text="Date:",font=("verdana",12,"bold"),fg="navyblue",bg="#4682b4")
        ...
        date_entry = DateEntry(left_frame, width=20, textvariable=self.var_date, background='darkblue', foreground='white', borderwidth=2,justify=CENTER)
        date_entry.grid(row=2,column=1,padx=5,pady=5,sticky=W)
        

        #Attendance
        student_attend_label = Label(left_frame,text="Attend-status:",font=("verdana",12,"bold"),fg="navyblue",bg="#4682b4")
        ...
        time_label = Label(left_frame,text="Semester:",font=("verdana",12,"bold"),fg="navyblue",bg="#4682b4")
        time_label.grid(row=3,column=0,padx=5,pady=5,sticky=W)

        time_entry = ttk.Entry(left_frame,textvariable=self.var_semester,width=15,font=("verdana",12,"bold"))
        time_entry.grid(row=3,column=1,padx=5,pady=5,sticky=W)

        # time_label = Label(left_frame,text="Department:",font=("verdana",12,"bold"),fg="navyblue",bg="#4682b4")
        # time_label.grid(row=3,column=2,padx=5,pady=5,sticky=W)

        # time_entry = ttk.Entry(left_frame,textvariable=self.var_department,width=15,font=("verdana",12,"bold"))
        # time_entry.grid(row=3,column=3,padx=5,pady=5,sticky=W)

        # ===============================Table Sql Data View==========================
        table_frame = Frame(left_frame,bd=2,bg="#4682b4",relief=RIDGE)
        table_frame.place(x=10,y=140,width=635,height=260)
        ...
        #create table 
        self.attendanceReport_left = ttk.Treeview(table_frame,column=("ID","Roll_No","Name","Time","Date","Attend","Semester"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        #Button Frame
        btn_frame = Frame(left_frame,bd=2,bg="#4682b4",relief=RIDGE)
        btn_frame.place(x=10,y=400,width=635,height=50)
        ...
        # Right Label Frame 
        right_frame = LabelFrame(main_frame,bd=2,bg="#4682b4",relief=RIDGE,text="Student Attendance",font=("verdana",12,"bold"),fg="navyblue")
        ...
        table_frame = Frame(right_frame,bd=2,bg="#4682b4",relief=RIDGE)
        table_frame.place(x=10,y=100,width=635,height=350)
        ...
        #create table 
        self.attendanceReport = ttk.Treeview(table_frame,column=("ID","Roll_No","Name","Time","Date","Attend","Semester"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
    ...
    # =================================update for mysql button================
         ...
         #Searching System in Right Label Frame 
        # search_frame = LabelFrame(right_frame,bd=2,bg="#4682b4",relief=RIDGE,text="Search System",font=("verdana",12,"bold"),fg="navyblue")
        # search_frame.place(x=10,y=5,width=635,height=80)

        #Phone Number
        search_label = Label(right_frame,text="Search:",font=("verdana",12,"bold"),fg="navyblue",bg="#4682b4")
        ...
        search_btn=Button(right_frame,command=self.search_data,text="Search",width=9,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        search_btn.grid(row=0,column=3,padx=5,pady=5,sticky=W)

        showAll_btn=Button(right_frame,command=self.fetch_data,text="Show All",width=8,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        showAll_btn.grid(row=0,column=4,padx=5,pady=5,sticky=W)

            #Update button
        del_btn=Button(right_frame,command=self.update_data,text="Update",width=9,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        del_btn.grid(row=1,column=1,padx=15,pady=5,sticky=W)
    #Update button
        del_btn=Button(right_frame,command=self.delete_data,text="Delete",width=8,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        del_btn.grid(row=1,column=2,padx=15,pady=5,sticky=W)


# ===========================Search Data===================
    def search_data(self):
        if self.var_search.get()=="":
            # or self.var_searchTX.get()=="Select"
            messagebox.showerror("Error","Select Combo option and enter entry box",parent=self.root)
        else:
            try:
                conn = mysql.connector.connect(host='localhost', username='root', password='root@raj123', database='face_recognition')
                my_cursor = conn.cursor()
                sql = "SELECT std_id,std_roll_no,std_name,std_time,std_date,std_attendance,std_semester FROM stdattendance where std_semester=" +str(self.var_search.get()) + "'"
 ...
 # ===========================Fetch data form database to table ================================
    ...
    def update_data(self):
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status" or self.var_semester.get()=="":
            ...
            try:
                ...
                if Update > 0:
                    conn = mysql.connector.connect(host='localhost', username='root', password='root@raj123',database='face_recognition')
                    mycursor = conn.cursor()
                    mycursor.execute("update stdattendance set std_id=%s,std_roll_no=%s,std_name=%s,std_time=%s,std_date=%s,std_attendance=%s,std_semester=%s where std_id=%s",( 
                    self.var_id.get(),
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_time.get(),
    ...
    def get_cursor_left(self,event=""):
        ...
        # self.var_department.set(data[7])  
    ...
    def action(self):
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                conn = mysql.connector.connect(host='localhost', username='root', password='root@raj123', database='face_recognition')
                mycursor = conn.cursor()
                mycursor.execute("insert into stdattendance values(%s,%s,%s,%s,%s,%s,%s)",(
                self.var_id.get(),
                self.var_roll.get(),
                self.var_name.get(),
                self.var_time.get(),
